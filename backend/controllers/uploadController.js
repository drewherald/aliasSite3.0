...
const uploadWorkRequest = async (req, res) => {
  ...
  try{
    ...
    const mailOptions = {
      ...
      subject: 'Work Request uploaded',
      text: `Work request uploaded by ${email}. \nRequest Type: ${requestType}. \nRequest Body: ${requestBody}.`,
    };

    await transporter.sendMail(mailOptions);
  

    res.status(200).json({ email, requestType: request.requestType, requestBody: request.requestType, message: "Request sent successfully! We will reach out soon." });
  } catch (e) {
    res.status(400).json({  error: `Request failed. Please try again or contact your admin with these details: ${e.message}`  });
  }
}
